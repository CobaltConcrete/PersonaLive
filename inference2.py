...
def run_webcam(args, config, pipe, face_mesh, streamer):
    ...
    try:
        while True:
            ...
            # Consume the oldest chunk_size frames; no overlap so the buffer
            # drains cleanly and we always feed exactly chunk_size frames.
            ...
            for pil_frame in chunk:
                ...
                try:
                    ...
                    # Face detection failed — fall back to resized full frame
                    dri_faces.append(pil_frame.resize((256, 256)))

            # temporal_window_size and temporal_adaptive_step must always be 4
            # to match the internal frame arithmetic in the denoising UNet.
            gen_video = pipe(
                ori_pose_images,
                ref_image_pil,
                dri_faces,
            ...
            ).videos  # (1, C, F, H, W)  values in [0, 1]
